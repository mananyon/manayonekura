...
/* ---------- 2 プロフィール ---------- */
s=p.addSlide(); bg(s); eyebrow(s,'01','PROFILE');
...
card(s,8.81,1.9,3.78,2.5,'研究','学会発表・産学連携','全日本鍼灸学会でうつ・不安症状の実態調査を継続報告。理化学研究所・東京有明医療大学・玉川大学・ケアクルとの共同研究に参画。');
hr(s,M,4.85,CW);
const stats=[['11年目','鍼灸院 開業'],['20,000人+','のべ臨床患者数'],['3年連続','全日本鍼灸学会 発表'],['修士','武蔵野大学大学院 人間学']];
stats.forEach((st,i)=>{
  const x=M+i*(CW/4);
...
foot(s,'0:20 → 1:00');
s.addNotes('三重で鍼灸院を11年。のべ2万人以上を診てきました。臨床だけでなく、鍼灸師の育成と、学会での研究発表も続けています。心の不調を扱う鍼灸師として、現場・人・データの3つを持っているのが私の立ち位置です。');

/* ---------- 3 強み ---------- */
s=p.addSlide(); bg(s); eyebrow(s,'02','STRENGTH');
...
title(s,'鍼灸には天井がある。だから「頭をオフにする」事業を始めます',26);
bullets(s,M,1.95,7.3,[
  [{text:'ねここち',b:1},{text:'／頭オフ事業。考えごとで止まらない頭を、強制的にオフにする時間を売ります。2026年 立ち上げ予定。'}],
  '鍼灸は国家資格者に依存し、施術者1人あたりの時間が上限になる。＝ 売上に天井がある。',
  'ねここちは資格に依存しない設計。未病層が「治療」と身構えずに入れる入口になる。',
  [{text:'入口＝ねここち／出口＝鍼灸・医療。',b:1},{text:'必要な人だけを、適切な先へ渡す導線を持てる。'}]
]);
...
s.addText([{text:'月次利益 30',options:{}},{text:'万円',options:{fontSize:17}}],
  {x:8.45,y:2.55,w:4.13,h:0.8,fontSize:34,fontFace:SERIF,bold:true,color:INK,align:'center',
   isTextBox:true,margin:0,valign:'middle'});
s.addText('想定値。多店舗展開を前提に設計しており、人と仕組みが揃えば積み上がる構造です。',
...
foot(s,'2:40 → 3:25');
s.addNotes('ただ、鍼灸だけでは天井があります。国家資格者の時間が上限になるからです。そこで「ねここち」という頭をオフにする事業を始めます。1店舗あたり月次利益30万円を想定。ねここちが入口、鍼灸や医療が出口という導線をつくります。');
...
/* ---------- 7 当社の課題（濃色） ---------- */
s=p.addSlide(); bg(s,true); eyebrow(s,'06','OUR LIMIT',true);
...
[['共同研究','開業鍼灸院でのメンタルヘルスの可能性を検証する共同研究','ここちめいど／東京有明医療大学／理化学研究所／ケアクル　― PR TIMES'],
 ...
 ['学会','全日本鍼灸学会（2024・2025・2026）／ 日本精神神経学会 鍼灸シンポジウム シンポジスト','うつと不安症状を有する患者の実態調査 第3報・第4報 ほか　― 2026.6 登壇予定']
].forEach((r,i)=>{
  const y=1.95+i*0.8;
  hr(s,M,y-0.12,CW);
