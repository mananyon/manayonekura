...
const PAPER='F3EADB', INK='2B2620', INK2='5A5249', INK3='8B8175', RULE='D2C4AE',
      SHU='9C5446', MIDORI='5E7060', CARD='FBF6EC',
...
function eyebrow(s,num,label,dark){
  s.addShape(p.ShapeType.rect,{x:M,y:0.615,w:0.26,h:0.014,fill:{color:dark?SHU:SHU},line:{type:'none'}});
  const runs=[];
  if(num) runs.push({text:num+'   ',options:{fontFace:SERIF,color:dark?SHU:SHU,bold:true,charSpacing:2}});
  runs.push({text:label,options:{color:INK3,charSpacing:3}});
...
function title(s,t,dark,size){
  s.addText(t,{x:M,y:0.85,w:CW,h:0.85,fontSize:size||28,fontFace:SERIF,bold:true,
    color:INK,isTextBox:true,margin:0,valign:'middle',charSpacing:1});
}
function foot(s,time,dark){
  s.addText('米倉まな｜アポプラスステーション様 ご提案',{x:M,y:6.93,w:8,h:0.3,fontSize:12,fontFace:SANS,
    color:INK3,isTextBox:true,margin:0,valign:'middle',charSpacing:1});
  s.addText(time,{x:W-M-3.5,y:6.93,w:3.5,h:0.3,fontSize:12,fontFace:SANS,color:INK3,
...
function src(s,t,dark,y){
  s.addText('出典：'+t,{x:M,y:y||6.42,w:CW,h:0.5,fontSize:11,fontFace:SANS,
    color:INK3,isTextBox:true,margin:0,valign:'top',lineSpacing:15});
}
...
const pills=['認定鍼灸師（開業12年目）','はりきゅう処ここちめいど 院長','オンラインサロン ここちめいど 主宰','ここちはり 代表','武蔵野大学大学院 人間学 修士'];
let px=M, py=5.08;
pills.forEach(t=>{
...
/* ---------- 2 原点 ---------- */
s=p.addSlide(); bg(s); eyebrow(s,'01','ORIGIN');
title(s,'私は2013年ごろまで、患者の側にいました');
[['01',[{t:'精神疾患の当事者として、'},{t:'約8年間さまよいました。',b:1}]],
 ['02',[{t:'病院に通っても、よくならない時期が長く続きました。'}]],
 ['03',[{t:'最後に私を助けたのが、鍼灸でした。'}]]
].forEach((f,i)=>{
  const y=2.2+i*0.85;
  s.addText(f[0],{x:M,y,w:0.5,h:0.4,fontSize:16,fontFace:SERIF,bold:true,color:SHU,isTextBox:true,margin:0,valign:'middle'});
  s.addText(f[1].map(r=>({text:r.t,options:{color:r.b?INK:INK2,bold:!!r.b}})),
...
s.addText('「もっと早く、\n鍼灸に出会いたかった」',{x:7.2,y:2.15,w:5.38,h:1.5,fontSize:24,fontFace:SERIF,bold:true,
  color:INK,isTextBox:true,margin:0,valign:'top',lineSpacing:40});
s.addText('患者さんから、いちばん多く言われる言葉です。東洋医学や鍼灸という手段、未病という概念も、その一つです。この「もっと早く」をなくすのが私の仕事です。',
  {x:7.2,y:3.85,w:5.38,h:1.0,fontSize:14,fontFace:SANS,color:INK2,isTextBox:true,margin:0,valign:'top',lineSpacing:19});
foot(s,'0:20 → 0:50');
...
card(s,8.81,1.85,3.78,2.8,'研究','学会発表・産学連携','全日本鍼灸学会・日本うつ病学会で発表し、日本東洋医学会、日本精神神経学会にも招待講演にて登壇。理化学研究所や大学との共同研究に参画。');
hr(s,M,4.95,CW);
const stats=[['12年目','鍼灸院 開業'],['20,000人+','のべ臨床患者数'],['4学会','発表・登壇'],['修士','武蔵野大学大学院 人間学']];
stats.forEach((st,i)=>{
  const x=M+i*(CW/4);
...
foot(s,'0:50 → 1:20');
s.addNotes('三重で鍼灸院を12年。のべ2万人以上を診てきました。臨床だけでなく、鍼灸師の育成と、学会での研究発表も続けています。全日本鍼灸学会と日本うつ病学会で発表し、日本精神神経学会にも登壇しました。心の不調を扱う鍼灸師として、現場・人・データの3つを持っているのが私の立ち位置です。');
...
/* ---------- 5 打ち手 ---------- */
s=p.addSlide(); bg(s); eyebrow(s,'04','ACTION');
...
/* ---------- 6 先行事例 ---------- */
s=p.addSlide(); bg(s); eyebrow(s,'05','CASE');
...
quote(s,7.7,1.95,4.88,3.5,'ここから私が受け取ったこと',[
  ...
  {text:'ブレインスリープは「睡眠」から入った。私は「こころ」から入ります。',options:{bold:true,color:INK}}]);
...
/* ---------- 7 課題感 ---------- */
s=p.addSlide(); bg(s); eyebrow(s,'06','ISSUE');
title(s,'これは私の実感であり、社会の数字でもあります');
s.addText('社会で起きていること',{x:LX,y:1.78,w:LW,h:0.28,fontSize:14,fontFace:SANS,bold:true,color:SHU,charSpacing:2,isTextBox:true,margin:0,valign:'middle'});
s.addText('私が現場で見ていること',{x:RX,y:1.78,w:RW,h:0.28,fontSize:14,fontFace:SANS,bold:true,color:MIDORI,charSpacing:2,isTextBox:true,margin:0,valign:'middle'});
const rows=[
  [[{text:'約600万人　',f:1},{text:'精神疾患を持つ患者数。国は2024年、厚生労働白書で初めて「こころの健康」をテーマに。'}],
  ...
  [[{text:'年7.6兆円　',f:1},{text:'メンタル不調による経済損失。休職・欠勤（アブセンティーイズム）は0.3兆円。残り7.3兆円は出社している人（プレゼンティーイズム）です。'}],
   '休職や通院の手前で止まる方が、いちばん多い。この層は誰の担当でもありません。']
];
rows.forEach((r,i)=>{
  const y=2.10+i*1.15;
  hr(s,LX,y-0.14,LW); hr(s,RX,y-0.14,RW);
  s.addText(r[0].map(t=>({text:t.text,options:t.f?{fontFace:SERIF,fontSize:17,bold:true,color:SHU}:{color:INK}})),
    {x:LX,y,w:LW,h:1.1,fontSize:14,fontFace:SANS,isTextBox:true,margin:0,valign:"top",lineSpacing:22});
  s.addText(r[1],{x:RX,y,w:RW,h:1.05,fontSize:14,fontFace:SANS,color:INK2,isTextBox:true,margin:0,valign:'top',lineSpacing:22});
});
...
src(s,'厚生労働省 患者調査／令和6年版 厚生労働白書／改正労働安全衛生法（2025年5月公布・2028年4月施行）／メンタルヘルス不調による経済損失の試算',false,6.4);
foot(s,'3:05 → 3:50');
s.addNotes('ここからは、私が現場で感じていることをお話しします。左が社会の数字、右が私が院で見ていることです。精神疾患の患者は約600万人、国も2024年に初めて白書のテーマにこころの健康を据えました。私の院でも、心の不調を主訴に来られる方が明らかに増えています。2028年からは50人未満の会社にもストレスチェックが義務化されます。ただ、検査で気づいても次に行く場所がない。病院はもう新規を受けきれていません。そして損失の7.6兆円のうち、休職や欠勤によるアブセンティーイズムはわずか0.3兆円です。残りの7.3兆円は、出社しているのに力が出ていないプレゼンティーイズムの分。つまり損失のほとんどは、休んでいない人が生んでいます。この層が、いま誰の担当にもなっていない。だから私は、未病の段階で止めたいと考えています。');

/* ---------- 8 当社の課題 ---------- */
s=p.addSlide(); bg(s); eyebrow(s,'07','MY LIMIT');
title(s,'私の課題：私1人で抱えられる量に天井があります');
[['01','臨床も、育成も、研究も、いまは私に紐づいている（属人）'],
 ['02','店舗は増やせても、施術者と運営を担える人は増やせていない'],
 ['03','課題の大きさに対して、届けられる人数が圧倒的に足りない']].forEach((f,i)=>{
  const y=2.25+i*0.85;
  s.addText(f[0],{x:M,y,w:0.5,h:0.4,fontSize:16,fontFace:SERIF,bold:true,color:SHU,isTextBox:true,margin:0,valign:'middle'});
  s.addText(f[1],{x:M+0.6,y,w:5.4,h:0.7,fontSize:14,fontFace:SANS,color:INK,isTextBox:true,margin:0,valign:'middle',lineSpacing:19});
 });
s.addText('仕組みと、人と、\n座組みが要る。',{x:7.2,y:2.2,w:5.38,h:1.5,fontSize:26,fontFace:SERIF,bold:true,
  color:INK,isTextBox:true,margin:0,valign:'top',lineSpacing:42});
s.addText('ここは私が最も苦手な領域であり、御社が強い領域だと考えています。',
  {x:7.2,y:3.95,w:5.38,h:0.9,fontSize:14,fontFace:SANS,color:INK2,isTextBox:true,margin:0,valign:'top',lineSpacing:19});
foot(s,'3:50 → 4:10');
...
s.addImage({path: path.join(__dirname,'flow.png'), x:M, y:1.8, w:CW, h:CW*246/1060});
quote(s,M,4.75,CW,1.45,'設計',[
  {text:'記録して終わりにしない。返事があるから続き、続くからデータになる。',options:{bold:true,color:INK}},
  {text:'これがほかの記録アプリとの違いです。玉川大学 柴田研究室との共同提案で、私は臨床監修を担当します。'}]);
foot(s,'4:10 → 4:55');
...
/* ---------- 10 なぜ御社と ---------- */
s=p.addSlide(); bg(s); eyebrow(s,'09','STRATEGIC FIT');
title(s,'3つの壁が、御社では最初から解けています');
s.addText('立ちはだかる壁',{x:LX,y:1.78,w:LW,h:0.3,fontSize:14,fontFace:SANS,bold:true,color:SHU,charSpacing:2,isTextBox:true,margin:0,valign:'middle'});
s.addText('御社がすでに持っているもの',{x:RX,y:1.78,w:RW,h:0.3,fontSize:14,fontFace:SANS,bold:true,color:MIDORI,charSpacing:2,isTextBox:true,margin:0,valign:'middle'});
...
title(s,'だから、ディスカッションしたいと感じました');
[['01','未病領域の新規事業を、共同でディスカッションさせてください','未病領域に小さいながら関わるなかで、ターゲットと課題を実感し、データ化しています。'],
...
quote(s,M,5.0,CW,1.45,'本日お願いしたいこと',[{text:'生きづらさを抱えた人を、治療が必要になる手前で減らしたい。'},{text:'まずは本日、このうちどこに可能性がありそうか、率直なご意見をいただけたら嬉しいです。',options:{bold:true,color:INK}}],SHU);
foot(s,'5:35 → 6:00');
